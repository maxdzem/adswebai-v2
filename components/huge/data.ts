 ...
 * надо.
 *
 * Картинки лежат локально в /public/huge (webp). Раньше они тянулись
 * прямо с Cloudinary Huge — страница зависела от чужого сервера и падала
 * бы вместе с ним. Файлы скачаны один раз и закоммичены.
 *
 * Строки ниже остались прежними — это исходные пути Cloudinary, они
 * работают как идентификаторы. cld() берёт из пути последний сегмент и
 * превращает его в путь к локальному файлу. Чтобы поставить свою
 * картинку, положите её в /public/huge и укажите имя файла.
 */

/**
 * Путь к локальной картинке. Аргумент — исходный путь Cloudinary; берётся
 * его последний сегмент, он же имя файла в /public/huge.
 *
 * Второй аргумент (ширина) сохранён, чтобы не переписывать все вызовы:
 * ширину применили при скачивании, в рантайме она уже не нужна.
 */
const cld = (path: string, _width = 1920) => {
  const name = path.split("/").pop()!.replace(/\.(png|jpe?g|webp)$/i, "");
  return `/huge/${name}.webp`;
};
